...
********************************************************************************************			
* 1. Ratio of Taxable Budget Shares of Top to Bottom Quintile: former figure 6 
********************************************************************************************							
	...
	*** Fig1: Only Non-Food Taxed 
	...
	name(G1, replace);	
	gr export "$main/graphs/calibration/bs_nf.${format}", replace as(${format}) 	; 
	...
	*** Fig2: Only Formal Taxed 
	...
	legend(off)
	name(G2, replace);
	gr export "$main/graphs/calibration/bs_for.${format}", replace as(${format}) 	; 
	...
	*** Fig3: Only Formal non-food taxed
	...
	legend(off) 
	name(G3, replace);	
	gr export "$main/graphs/calibration/bs_for_nf.${format}", replace as(${format}) 	; 
...
************************************************************************************
	...
	* EXTRAS  
	* Savings Rates now let's assume their are country specific
	merge m:1 country_code decile using "$main/proc/saving_rates.dta"
	gen inc_total_sim = exp_total0  / (1-saving_rate_country_decile)
	
	*old way to compute savings
...
*********************************************		
	...
	****************************************************************************************************	
	...
	local yaxis "ylabel(0.04(0.02)0.14, nogrid labsize(`size')) yline(0.04(0.02)0.15, lstyle(minor_grid) lcolor(gs1)) " 
	...
	legend(order(1 "Uniform rate, only formal taxed" 2 "Food exempt, only formal taxed" 3  "Food exempt, formal & informal taxed") pos(12) ring(0) col(1) size(small))
	`yline'
	`xaxis' 
	`yaxis' 
	`xtitle' 
	...
	`graph_region_options'
	name(G, replace) ; 
	 gr export "$main/graphs/cross_country/simulations/effective_rates_decile_${scenario}.${format}", replace ; 
	...
	#delim ;
	tw 
	connected t_eff_food1_inf1_vat0_B1 decile, lcolor(black) mcolor(black)  , 
	legend(order(1 "Uniform rate, only modern taxed" ) pos(12) ring(0) col(1) size(small)) legend(off)
	`yline'
	`xaxis' 
	`yaxis' 
	`xtitle' 
	...
	`graph_region_options'
	name(G, replace) ; 
	 gr export "$main/graphs/cross_country/simulations/effective_rates_decile_slide0_${scenario}.${format}", replace ; 	
	
	#delim ;
	tw 
	connected t_eff_food1_inf0_vat0_B1 decile, lcolor(red) msymbol(o) mcolor(red)  , 
	legend(order(1 "Uniform rate, only modern taxed" ) pos(12) ring(0) col(1) size(small)) legend(on)
	`yline'
	`xaxis' 
	`yaxis' 
	`xtitle' 
	...
	`graph_region_options'
	name(G, replace) ; 
	 gr export "$main/graphs/cross_country/simulations/effective_rates_decile_slide1_${scenario}.${format}", replace ; 

	#delim ;
	tw 
	connected t_eff_food1_inf0_vat0_B1 decile, lcolor(red) msymbol(o) mcolor(red) || 
	connected t_eff_food0_inf1_vat0_B1 decile, lcolor(green) msymbol(X) mcolor(green)  , 
	legend(order(1 "Uniform rate, only modern taxed" 2 "Food exempt, modern & traditional taxed") pos(12) ring(0) col(1) size(small))
	`yline'
	`xaxis' 
	`yaxis' 
	`xtitle' 
	...
	`graph_region_options'
	name(G, replace) ; 
	gr export "$main/graphs/cross_country/simulations/effective_rates_decile_slide2_${scenario}.${format}", replace ;  
	...
	connected t_eff_food0_inf1_vat0_B1 decile, lcolor(green) msymbol(X) mcolor(green)  ||
	connected t_eff_food0_inf0_vat0_B1 decile, lcolor(orange) msymbol(s) mcolor(orange)  , 
	legend(order(1 "Uniform rate, only modern taxed" 2 "Food exempt, modern & traditional taxed" 3 "Food exempt, only modern taxed") pos(12) ring(0) col(1) size(small))
	`yline'
	`xaxis' 
	`yaxis' 
	`xtitle' 
	...
	`graph_region_options'
	name(G, replace) ; 
	 gr export "$main/graphs/cross_country/simulations/effective_rates_decile_slide3_${scenario}.${format}", replace ; 	 
	...
	replace `var'_1_2 = `var'_1_2[_n-1] if missing(`var'_1_2)
	replace `var'_9_10 = `var'_9_10[_n-1] if missing(`var'_9_10)
	...
	matrix results[3,4] = `r(mean)' 
	*/
	use "$main/waste/cross_country/effective_rates_decile_inputvat_specific.dta", clear
	...
	global writeto "$main/tables/cross_country/tableA6_new.tex"
	cap erase $writeto
