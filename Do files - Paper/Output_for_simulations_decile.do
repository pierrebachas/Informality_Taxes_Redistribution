...
*********************************************	
	...
	* Generates a dataset which stores the coefficients from the regressions	 
	local filename "output_for_simulation_dcl"
...
******************************************************		
	*** SAMPLE 	
	...
	// keep if country_code == "CD"			// To run on one country only select its acronym  
...
*********************************************					
	...
	forval k=1/`n_models' {
		...
		* Gen total exp deciles 
		drop decile_income 
		gen decile_income = . 
		forvalues i = 10(10)90 { 	
			_pctile total_exp  [aw= hh_weight], p(`i')  
			local p_low = `r(r1)'
			replace decile_income = `i'/10 if total_exp < `p_low' & decile_income == . 
			} 
