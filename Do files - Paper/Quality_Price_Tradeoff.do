...
*********************************************					
	...
	if `graph' { 
		...
		* Food ENGEL curve:
		#delim ;
		twoway (lpolyci share_quality log_income_pp if log_income_pp>= `bottom_range' & log_income_pp <= `top_range' [aw = hh_weight], clcolor(navy)),
		...
		graph export "$main/graphs/cross_country/Food_Engel_Curves/${country_code}_quality_price.${format}", replace ;		
