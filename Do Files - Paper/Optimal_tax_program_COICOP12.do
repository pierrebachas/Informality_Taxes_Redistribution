...
***************************************************
	...
	set more off
	cap log close
	log using "$main/logs/optimal_tax_COICOP12.log", replace
...
***************************************************
	...
	** Saving non food aggregate elasticities obtained by baseline program
	...
	qui optimal_tax `1' 1 10 0 0 1 1 0.7 `9' 1 0 0 0
...
{	
	...
	*** Merge to get Formal Food and non Food Slopes 
	// Note: this is not needed, it can just serve as a comparison to the above to be sure we are consistent on how me measure our Engel curve slopes
	/*
	
	preserve	
	use "$main/proc/regressions_inffood_`1'.dta", clear	
	keep if iteration == 3 | iteration == 1
	keep country_code b2 b4 iteration 
	reshape wide b2 b4 , i(country_code) j(iteration)		
	
	rename b23 b_f_formal 
	rename b43 b_nf_formal
	
	rename b21 s_f_formal 
	rename b41 s_nf_formal
	replace s_f_formal  = s_f_formal * 100 
	replace s_nf_formal = s_nf_formal * 100 
	
	tempfile temp_data_IECslopes_foodnonfood
	save `temp_data_IECslopes_foodnonfood' , replace 
	restore	
	
	merge m:1 country_code using `temp_data_IECslopes_foodnonfood'
	drop _merge		
	*/ 
