...
*********************************************	
	...
	global scenario "vat_input"    // central , proba , robust: see definition in Master.do
...
*********************************************	
	...
	***********************************************************		
	...
	if `share_informality'	{
		...
		`yaxis' 
		`xtitle' 
		ytitle("Budget Share", margin(medsmall) size(`size'))
		legend(off) 	
		...
		graph export "$main/graphs/cross_country/level_informality_$scenario.${format}", replace	;
	...
	* Define a local with the set of variables to run this over 
	...
	local slope_informality_fits 	= 1		// Average Informality on country log_gdp_pc
	...
	local slope_geo					= 0     // Shows sloped when controlling for urbban vs rural and for survey blocks
...
if `slope_informality_fits' {
		...
		graph save "$main/graphs/cross_country/slope_informality_fits_$scenario.gph", replace	;
		graph export "$main/graphs/cross_country/slope_informality_fits_$scenario.pdf", replace	;
...
if `slope_informality_ci' {
		...
		`yaxis' 
		`xtitle'		
		ytitle("Engel Curve Slope", margin(medsmall) size(`size'))
		legend(off) 	
		`graph_region_options'	
		name(slope_informality, replace) 		; 
		
		graph export "$main/graphs/cross_country/slope_informality_ci_$scenario.${format}", replace	;
...
if `slope_COICOP' {
	forvalues i = 11(1)15 { 
		...
		graph export "$main/graphs/cross_country/slope_b`i'_$scenario.${format}", replace	;
