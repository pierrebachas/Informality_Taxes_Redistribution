...
*********************************************		
	...
	***************
	...
	if "$country_code"=="CL" {
	import delimited "/Users/evadavoine/Dropbox/Regressivity_VAT/Stata/tables/Chile2017/CL_TOR_stats_for_crosswalk.csv", clear 
	rename tor_original TOR_original
	rename tor_original_name TOR_original_name
	}
	
	else if "$country_code"!="CL" {
	import excel "$main/tables/$country_fullname/${country_fullname}_TOR_stats_for_crosswalk.xls",  firstrow clear
	}
	
	destring pct_expenses, force replace

	gen country="$country_fullname"
	keep country pct_expenses TOR_original_name detailed_classification   
	

	rename pct_expenses pct 
	label var pct "$\%$"
	rename TOR_original_name Original_name
