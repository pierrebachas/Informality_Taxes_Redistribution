...
*********************************************
	...
	local stats_for_country_loop = 0		// Generates separation categories 3 and 4 for country_loop
	local stats_for_country_loop_MZ = 0		// Generates separation categories 3,4,5 for MZ for country_loop
	...
	local layers_graph_adjusted = 0 	// Figure in PAPER with the different layers, separated formal vs informal with the adjustments for categories 3 and 4
	...
	**** Exlcude these countries ("ZA" "TZ" "TN" "ME" "MZ" "CD" ) and then do the curve with share of category 3 by decile of total 3+4
	** This then feeds into the "$main/data/Country_index_for_stata_loop.xlsx" 
	...
	* Merge with Layers data 
	...
	gen log_GDP_pc = ln(GDP_pc_constantUS2010)
	...
	foreach name in "ZA" "TZ" "TN" "ME" "MZ" "CD" { 
		sum log_GDP_pc if country_code == "`name'" 
		local min_`name' = `r(mean)' - 1
		local max_`name' =  `r(mean)' + 1
		gen sample_`name' = 0 
		replace sample_`name' = 1 if log_GDP_pc >= `min_`name'' & log_GDP_pc <= `max_`name'' 
	}
	
	drop if inlist(country_code, "ZA" "TZ" "TN" "ME" "MZ" "CD" )  // Note Mozambique should have its own treatment since both 5 and 3 are missing and not only 3 
	...
	*** CAN NOW GENERATE THE PARAMETERS WE NEED BY COMPARING THE 4 COUNTRIES WITH MISSING INFO with their peers (countries which are within 1 log income distance) 
	...
	************************************************************************************************************		
	* PRELIM. Generate level and slopes for MZ which only has level 4 but neither 3 nor 5
	************************************************************************************************************		
	
	if `stats_for_country_loop_MZ' {
	* Prepare GDP data 
	tempfile gdp_data	
	import excel using "$main/data/Country_information.xlsx" , clear firstrow	
	...
	* Take average across countries 
	...
	forvalues i = 1(1)8 {  
	#delim ;  
		twoway (connected mean_sharelevel decile if group_level == `i' , mcolor(navy)) , 
		`xaxis' 
		`yaxis' 
		// `xtitle' 
	...
	graph export "$main/graphs/cross_country/levels_share_combined_G`i'.${format}", replace		; 
	#delim cr 	
		}
	...
	** Gen levels
	...
	graph export "$main/graphs/cross_country/shaded_informality_levels.${format}", replace;		
	...
	** Gen levels
	...
	graph export "$main/graphs/cross_country/shaded_formality_levels.${format}", replace;	
