...
* Note, all programs have to be changed in the following way: 
					* name of program is now: optimal_tax_pit, name of dofile is Optimal_tax_pit_program
					* option 3 (welfare weights) change from 10 to 11
					* add option 14: if the PIT interacts or not (so for these scenarios that currently exist it involves adding a zero
	...
	/* OPTIMAL TAX 
		...
		1: scenario used to construct the dataset (central/specific/proba)
	...
	*** Extensions:  	
	...
	14: =1 if allow for PIT
		...
		specific			   specific 	1 10 0 0 1 1 0.7 1.5 1 0 0 0	
...
******************
	...
	local baseline 			= 1		// Baseline		  	  	central  1 11 0 0 1 1 0.7 1.5 1 0 0 0 
	local baseline_VAT 		= 1		// Baseline + VAT	   	central  1 11 0 0 1 1 0.7 1.5 1 0 0 0.07 	
	local baseline_savings 	= 1		// Baseline + Savings 	central  1 11 0 0 1 1 0.7 1.5 1 0 0.015 0 
	local proba				= 1     // Proba			  	proba 	 1 11 0 0 1 1 0.7 1.5 1 0 0 0 
	local specific			= 1		// Country-specific		specific 1 11 0 0 1 1 0.7 1.5 1 0 0 0
	local baseline_e_c_1 	= 1		// Baseline, e_c = 1 	central  1 11 0 0 1 1 0.7 1 1 0 0 0 
	local baseline_e_c_2    = 1		// Baseline, e_c = 2	central  1 11 0 0 1 1 0.7 2 1 0 0 0 
...
if `baseline' {
	...
	qui optimal_tax central 1 11 0 0 1 1 0.7 1.5 1 0 0 0
	...
	** Tax Revenue collected in each scenario 
	forvalues i = 1(1)3 {
		...
		gen exp_total0_post_redist`i' = (1-taxed_share`i') * exp_total0 + (tax_rev`i' / 100)		
		drop tmp_tax_rev_`i' 
		}
...
if `robust' {
	...
	qui optimal_tax specific 1 10 0 0 1 1 0.7 1.5 1 0 0 0
	...
	* magnitudes of tax rates
	...
	merge 1:m country using "$main/waste/output_for_simulation_COICOP2_specific.dta"
	drop _merge 
...
{	
	...
	** Country by country changes in GINI coefficients 
	...
	save "$main/waste/cross_country/gini_specific.dta", replace 
}
}
****************************************************	
